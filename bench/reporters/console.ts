...
export function createConsoleReporter(): BenchmarkReporter {
  return {
    report(suite: BenchmarkSuite): string {
      ...
      lines.push(
        formatEnvironment(suite.environment)
          .split('\n')
          .map((l) => `  ${l}`)
          .join('\n'),
      );
      ...
      // Draw table
      lines.push(
        createLine(widths, TABLE.topLeft, TABLE.topT, TABLE.topRight, TABLE.horizontal),
      );
      lines.push(createRow(headers, widths, aligns));
      lines.push(
        createLine(widths, TABLE.leftT, TABLE.cross, TABLE.rightT, TABLE.horizontal),
      );
      ...
      const memAligns: ('left' | 'right' | 'center')[] = [
        'left',
        'right',
        'right',
        'right',
      ...
      lines.push(
        createLine(memWidths, TABLE.leftT, TABLE.cross, TABLE.rightT, TABLE.horizontal),
      );
      ...
      const lowSampleResults = suite.results.filter(
        (r) => r.success && r.time.count < 10,
      );
      ...
      if (lowSampleResults.length > 0) {
        ...
        for (const result of lowSampleResults) {
          lines.push(
            `  ${result.fixture}/${result.bundler}: low sample size (n=${result.time.count})`,
          );
        }
      }
